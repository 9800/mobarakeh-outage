...
public class OutageWidgetProvider extends AppWidgetProvider {
    ...
    static void render(Context context, AppWidgetManager mgr, int id) {
        ...
        v.setOnClickPendingIntent(R.id.widgetRoot, PendingIntent.getActivity(context, 0, open, pFlags()));

        Intent rf = new Intent(context, OutageWidgetProvider.class);
        rf.setAction(ACTION_REFRESH);
        v.setOnClickPendingIntent(R.id.wRefresh, PendingIntent.getBroadcast(context, 1, rf, pFlags()));

        SharedPreferences sp = context.getSharedPreferences("widget_cache", Context.MODE_PRIVATE);
        ...
        if (html == null) {
            ...
            if (m == null) {
                v.setTextViewText(R.id.wStatus, "⚠️ جدول معتبر دریافت نشد");
                v.setTextViewText(R.id.wRows, "");
            } else {
    ...
    static void fetchAsync(final Context context) {
        new Thread(new Runnable() {
            @Override public void run() {
                ...
                int[] ids = mgr.getAppWidgetIds(new ComponentName(context, OutageWidgetProvider.class));
                for (int id : ids) render(context, mgr, id);
            }
        }).start();
    }
